...
function validarPfx(pfxBase64: string, senha: string): Buffer {
  ...
  try {
    ...
    throw new Error(`Senha incorreta ou certificado invÃ¡lido: ${e.message}`);
  }
  return pfxBuf;
}

// httpsPost usa node:https diretamente â fetch nativo do Node 18+ nÃ£o suporta o parÃ¢metro agent
// necessÃ¡rio para mTLS (certificado A1 como client certificate)
function httpsPost(
  url: string,
  body: string,
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Fallback: busca cert do Supabase se nÃ£o veio no body
    if (!pfx_base64) {
      if (empresa_id) {
    ...
    // Valida o PFX via OpenSSL (nÃ£o usa node-forge â suporta todos os algoritmos ICP-Brasil)
    let pfxBuf: Buffer;
    try {
      pfxBuf = validarPfx(pfx_base64, pfx_senha);
    } catch (e: any) {
      return NextResponse.json(
        { status: "erro", mensagem: `Certificado A1 invalido: ${e.message}. Verifique se a senha estÃ¡ correta e recarregue o .pfx em /captura-sefaz.` },
        { status: 400 }
      );
    }
